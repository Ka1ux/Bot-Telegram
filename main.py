...
channel_link = "t.me/asiaticas18brasil"
token_bot = "6458594445:AAEbjUrS8fM0KlOwxfAMAfbLYFNJeRWI7_U"
...
for fold in folders:
    for i in os.listdir(f"{path_folder}{fold}/"):
        ...
        if after_dot in image:
            ...
            img.save(f'to_send/{fold}/asiaticas18brasil_{id}.{after_dot}')
            ...
            composite.write_videofile(f'to_send/{fold}/asiaticas18brasil_{id}.{after_dot}')



        if after_dot not in video + image:
            shutil.copyfile(f"{path_folder}{fold}{i}", f"{path}/to_send/asiaticas18brasil_{id}.{after_dot}")
